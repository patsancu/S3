...
function putObject(request, response, log, callback) {
    ...
    const backendType = locationConstraints[storageLocation].type;
    // TODO: Update to a standard across all Azure user metadata. For now, in
    // the context of replication to Azure, we do not want the prefix
    // 'x-amz-meta-' for the user metadata key.
    const metaHeaders = backendType === 'azure' ? {
...
function completeMultipartUpload(request, response, log, callback) {
    ...
    request.on('end', () => {
        ...
        if (userMetadata) {
            Object.assign(metaHeaders, JSON.parse(userMetadata));
        }
